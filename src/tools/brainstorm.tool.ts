...
const brainstormArgsSchema = z.object({
  prompt: z.string().min(1).max(100_000).describe("Primary brainstorming challenge or question to explore"),
  model: z.string().regex(/^[a-z0-9][a-z0-9.\-]*$/, "Invalid model name").max(64).optional().describe("Optional model override. Examples: 'gemini-2.5-flash', 'gemini-2.5-pro', 'gemini-3-flash-preview', 'gemini-3.1-pro-preview', 'gemini-2.5-flash-lite'. Defaults to gemini-3.1-pro-preview."),
  ...
  domain: z.string().max(200).optional().describe("Domain context for specialized brainstorming (e.g., 'software', 'business', 'creative', 'research', 'product', 'marketing')"),
  constraints: z.string().max(5000).optional().describe("Known limitations, requirements, or boundaries (budget, time, technical, legal, etc.)"),
  existingContext: z.string().max(50_000).optional().describe("Background information, previous attempts, or current state to build upon"),
  ideaCount: z.number().int().positive().max(50).default(12).describe("Target number of ideas to generate (default: 10-15)"),
